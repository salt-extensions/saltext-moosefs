...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/salt_moosefs/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/salt_moosefs/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/salt_moosefs/*"
            )
            ...
            #    "--include=src/saltext/salt_moosefs/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/salt_moosefs/config/schemas",
    )
